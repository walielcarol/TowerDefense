...
namespace ActionGameFramework.Health
{
	...
	public class Damager : MonoBehaviour
	{
        ...
        private void Start()
        {
            ...
            InvokeRepeating("GenerateMoveableTowerDamage", 0, 20f);

        }

        void GenerateMoveableTowerDamage()
        {
            float time = timeManager.time;


            factor = (time/20) * 0.1f + 0.2f;
            if(factor <2)
            {
                moveableTowerDamage = factor;
            }
            else{
                moveableTowerDamage = 1.5f;
            }


            if(gameObject.tag == "Projectile")
            {
                damage = moveableTowerDamage;
            }
